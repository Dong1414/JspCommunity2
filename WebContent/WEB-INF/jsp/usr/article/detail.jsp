...
<c:set var="pageTitle" value="${article.extra__boardName} 게시물" />
<c:set var="memberID" value="${article.memberId}" />
<%@ include file="../../part/head.jspf"%>
<script>
	$(function() {
		if (param.focusReplyId) {
			const $target = $('.reply-list-box tr[data-id="'
					+ param.focusReplyId + '"]');
		...
		<div class="article-detail">
			...
			<c:if test="${sessionScope.loginedMemberId == memberID}">
				<div class="mt-6 text-align-right">
					<a
						class="bg-gray-300 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-700 focus:ring-offset-2 focus:ring-offset-gray-300"
				...
				<c:if test="${!isLogined}">
					<div class="like flex flex-ai-c">
						<div class="flex flex-ai-c">
							<a class="btn btn-primary" href="#" onclick="if ( confirm('로그인 후 이용해주세요') == false ) { return false; }">							
								<span>
									<span>
						...
						<div class="flex flex-ai-c">
							<a class="btn btn-danger" href="#" onclick="if ( confirm('로그인 후 이용해주세요') == false ) { return false; }">
								<span>
									<span>
			...
			<div class="mb-6">
				<a href="list?boardId=${article.boardId}" class="bg-gray-300 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-700 focus:ring-offset-2 focus:ring-offset-gray-300">목록</a>
			</div>
		</div>
	</div>
</section>
					...
					<tr data-id="${reply.id}">
						...
						<c:if test="${!isLogined}">
								...
								<span class="response-list-box__likeOnlyPoint">
									<a href="#" onclick="if ( confirm('로그인 후 이용해주세요') == false ) { return false; }">								
										<i class="far fa-thumbs-up"></i>
									</a>
						...
						<td>
							<c:if test="${loginedMemberId == reply.memberId}">
								<div >
									<a class="hover:text-indigo-600 hover:font-bold font-semibold"
						...
						<td class="visible-sm-down">
							<div class="flex">
							<span class="response-list-box__writer response-list-box__writer--mobile">${reply.extra__writer}</span>
							<span>&nbsp;|&nbsp;</span>
							<div>
								<script type="text/x-template">${reply.body}</script>
								<div class="toast-ui-viewer"></div>
